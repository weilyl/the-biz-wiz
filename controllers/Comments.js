...
async getBusinessComments(req,res){
    const business_id=parseInt(req.params.business_id)
    try{
        let comments=await db.any("SELECT * FROM comments WHERE business_id=$1",
        business_id)
        return res.json(comments)
    }
    catch(err){
...
async getAComment(req,res){
    const comment_id=parseInt(req.params.id,10)
    try{
        let comment=await db.one("SELECT * FROM comments WHERE id=$1",
        comment_id)
...
async createComment (req, res) {
    try{
        await db.none("INSERT INTO comments (content,post_id,business_id) VALUES (${content},${post_id},${business_id})",
        req.body)
        return res.json({
            message: "success"
        })
